...
function appendDkaCoDiagnosisSteps(
  protocol: ConditionProtocol,
  coIds: string[]
): ConditionProtocol {
  if (!coIds.some((id) => id === 'septic_shock' || id === 'severe_pneumonia' || id === 'meningitis')) {
    return protocol;
  }
...
export function resolveDefinitiveCare(
  ...
  if (protocolId) {
    ...
    if (fellowshipId === 'dka') {
      protocol = appendDkaCoDiagnosisSteps(protocol, coIds);
    }
    ...
    const ctx: Omit<DefinitiveCareContext, 'allSteps'> = {
      ...
      protocol,
      fallbackSteps: [],
      coDiagnosisSteps: fellowshipId === 'dka' ? [] : coDiagnosisSteps,
      dischargeSteps: [],
