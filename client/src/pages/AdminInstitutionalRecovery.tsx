          ...
          {requests.map((r) => (
            <Card key={r.id}>
              ...
              <CardContent className="space-y-4">
                ...
                <InstitutionMatcher
                  requestId={r.id}
                  matchedId={matchIds[r.id] ? Number(matchIds[r.id]) : null}
                  onMatch={(id, name) =>
                    setMatchIds((prev) => ({ ...prev, [r.id]: id ? String(id) : "" }))
                  }
                  reviewNotes={notes[r.id] ?? ""}
                  onNotesChange={(text) => setNotes((prev) => ({ ...prev, [r.id]: text }))}
...
function InstitutionMatcher({
  ...
  onMatch: (id: number | null, name?: string) => void;
  reviewNotes: string;
  ...
  const { data } = trpc.institutionRecovery.searchInstitutions.useQuery(
    { query: search },
    { enabled: search.trim().length >= 2 }
  );

  const matches = data?.institutions ?? [];

  return (
    <div className="border-t pt-4">
      <p className="text-xs text-muted-foreground mb-2">
        Search and select the matching institution by name or registration number.
      </p>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        <div className="relative">
          {matchedId ? (
            <div className="flex items-center justify-between border rounded px-3 py-2 bg-slate-50 text-sm">
              <span className="font-medium text-slate-800">
                Matched: {selectedName || `ID ${matchedId}`}
              </span>
              <button
                type="button"
            ...
            <>
              <Input
                placeholder="Search institution name or reg. number..."
                value={search}
                ...
                <div className="absolute left-0 right-0 mt-1 bg-white border rounded-lg shadow-lg z-10 max-h-48 overflow-y-auto">
                  {matches.map((inst) => (
                    <button
                      ...
                      onClick={() => {
                        onMatch(inst.id, inst.companyName);
                        setSelectedName(`${inst.companyName} (${inst.registrationNumber || "No Reg #"})`);
                        setSearch("");
                      }}
                    >
                      <div className="font-semibold text-slate-800">{inst.companyName}</div>
                      <div className="text-xs text-slate-500">
                        ID: {inst.id} | Reg: {inst.registrationNumber || "—"}
                      </div>
                    </button>
                  ))}
                </div>
              ...
              {search.trim().length >= 2 && matches.length === 0 && (
                <div className="absolute left-0 right-0 mt-1 bg-white border rounded-lg p-3 text-xs text-slate-500 z-10">
      ...
      <div className="flex gap-3">
        <Button
          size="sm"
          onClick={onApprove}
          disabled={!matchedId || isPending}
        >
          Approve & Secure Account
        </Button>
        <Button
          size="sm"
          variant="outline"
