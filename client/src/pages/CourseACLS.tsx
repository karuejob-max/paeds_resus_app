...
/**
 * Learning path for ACLS program.
 * Pass ?enrollmentId= from Payment after checkout for direct continuation.
 */
export default function CourseACLS() {
  ...
  const aclsEnrollmentId = useMemo(() => {
    if (enrollmentIdFromUrl !== undefined) return enrollmentIdFromUrl;
    if (!rows) return undefined;
    return rows.filter((e) => e.programType === "acls").sort((a, b) => b.id - a.id)[0]?.id;
          ...
          <Card className="border-border shadow-sm">
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-xl">
                ...
                No ACLS enrollment found
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <p className="text-muted-foreground text-sm leading-relaxed">
                Enroll in <strong>ACLS</strong> first, complete payment, then return here. You can add{" "}
                <code className="text-xs bg-muted px-1.5 py-0.5 rounded">?enrollmentId=</code> to the URL after
                checkout.
              </p>
              <Link href="/enroll?courseId=acls">
                <Button variant="cta" className="w-full">
                  Go to enroll
                </Button>
              </Link>
    ...
    <div className="min-h-screen bg-brand-surface py-8 px-4">
      <div className="max-w-4xl mx-auto mb-8 flex flex-wrap items-center justify-between gap-4">
        ...
        <p className="text-sm text-muted-foreground max-w-md text-right leading-relaxed">
          Enrollment #{aclsEnrollmentId} · After payment, your certificate appears on the learner dashboard.
        </p>
      </div>
      ...
      <div className="max-w-4xl mx-auto mb-8 rounded-2xl border border-border bg-gradient-to-br from-card via-card to-brand-surface/50 p-6 md:p-8 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:gap-4">
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-primary">ACLS program</p>
            <h1 className="mt-1 text-xl font-bold text-foreground md:text-2xl">
              Advanced Cardiovascular Life Support
            </h1>
            <p className="mt-2 text-sm text-muted-foreground leading-relaxed">
              Work through each ACLS module and complete quizzes to track progress and certification readiness.
            </p>
          </div>
        </div>
      </div>

      <LearningPath enrollmentId={aclsEnrollmentId} programType="acls" />
    </div>
  );
}
