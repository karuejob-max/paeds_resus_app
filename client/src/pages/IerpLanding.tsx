import { ArrowRight, CheckCircle2, ClipboardCheck, GraduationCap, ShieldCheck, Users } from "lucide-react";
...
const phases = [
  {
    ...
    description: "Complete the platform cognitive sequence, then submit the two private AHA evidence documents for review.",
    icon: ClipboardCheck,
  },
  {
    number: "02",
    title: "Online simulations",
    description: "Build reliable team habits through confirmed Team Leader sessions and all six named Team Member roles.",
    icon: Users,
  },
  {
    number: "03",
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <section className="relative overflow-hidden border-b border-white/10 bg-[radial-gradient(circle_at_top_right,_rgba(45,212,191,0.22),_transparent_45%),linear-gradient(135deg,_#0f172a,_#172554_60%,_#0f766e)]">
        <div className="mx-auto grid max-w-6xl gap-12 px-6 py-20 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:py-28">
          <div>
            <p className="mb-5 text-xs font-semibold uppercase tracking-[0.28em] text-teal-200">Paeds Resus training pathway</p>
            <h1 className="max-w-3xl text-4xl font-semibold tracking-tight sm:text-6xl">Start your emergency readiness journey as an intern.</h1>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-200">The <strong>Intern Emergency Readiness Program</strong> helps interns practise the knowledge, roles, communication, and reassessment habits that make paediatric emergencies safer under pressure.</p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Button size="lg" className="bg-amber-400 text-slate-950 hover:bg-amber-300" onClick={startIerp}>
                Start IERP <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
              <Button size="lg" variant="outline" className="border-white/30 bg-transparent text-white hover:bg-white/10" onClick={() => document.getElementById("how-it-works")?.scrollIntoView({ behavior: "smooth" })}>
                See how it works
              </Button>
            </div>
          ...
          <div className="rounded-3xl border border-white/15 bg-white/10 p-6 shadow-2xl backdrop-blur-sm">
            <div className="flex items-center gap-3 border-b border-white/15 pb-5">
              <div className="rounded-2xl bg-teal-300/20 p-3"><GraduationCap className="h-7 w-7 text-teal-200" /></div>
              <div><p className="text-sm font-semibold text-teal-100">IERP at a glance</p><p className="text-xs text-slate-300">A staged, evidence-led pathway</p></div>
            </div>
      ...
      <section id="how-it-works" className="bg-slate-50 px-6 py-20 text-slate-950">
        <div className="mx-auto max-w-6xl">
          <div className="max-w-2xl"><p className="text-xs font-semibold uppercase tracking-[0.25em] text-teal-700">A dependable sequence</p><h2 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">Learn, rehearse, then demonstrate.</h2><p className="mt-4 text-slate-600">IERP keeps the learner journey explicit. Each phase explains what is complete, what is pending, and what must happen next.</p></div>
          <div className="mt-10 grid gap-5 lg:grid-cols-3">
            {phases.map(({ number, title, description, icon: Icon }) => <Card key={number} className="border-slate-200 bg-white shadow-sm"><CardHeader><div className="flex items-center justify-between"><span className="text-4xl font-semibold text-slate-200">{number}</span><Icon className="h-6 w-6 text-teal-700" /></div><CardTitle className="text-xl">{title}</CardTitle></CardHeader><CardContent><p className="text-sm leading-6 text-slate-600">{description}</p></CardContent></Card>)}
          </div>
        </div>
      </section>
