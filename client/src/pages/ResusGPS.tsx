...
export default function ResusGPS() {
  const { demographics, setDemographics, getWeightInKg } =
    usePatientDemographics();
  const [session, setSession] = useState<ResusSession>(() =>
    createSession(getWeightInKg(), demographics.age || null)
  );
  const [interventionPanelOpen, setInterventionPanelOpen] = useState(false);
  const [patientInfoOpen, setPatientInfoOpen] = useState(false);
  const [tempWeight, setTempWeight] = useState("");
  const [tempAgeValue, setTempAgeValue] = useState("");
  const [tempAgeUnit, setTempAgeUnit] = useState<
    "years" | "months" | "days" | "gestation_weeks"
  >("years");
  const [numberInput, setNumberInput] = useState("");
  const [numberInput2, setNumberInput2] = useState("");
  const [expandedThreat, setExpandedThreat] = useState<string | null>(null);
  const [reassessmentMode, setReassessmentMode] = useState<{
  ...
  useEffect(() => {
    ...
    const age = (demographics.age || "").toLowerCase().trim();

    const match = age.match(/(\d+(?:\.\d+)?)/);
    const value = match ? match[1] : "";

    if (age.includes("gest")) {
      setTempAgeUnit("gestation_weeks");
    } else if (age.includes("day")) {
      setTempAgeUnit("days");
    } else if (age.includes("month") || age.includes("mo")) {
      setTempAgeUnit("months");
    } else {
      setTempAgeUnit("years");
    }

    setTempAgeValue(value);
  }, [patientInfoOpen, demographics]);

  const buildAgeString = () => {
    if (!tempAgeValue) return null;
    const value = tempAgeValue.trim();
    switch (tempAgeUnit) {
      case "months":
        return `${value} months`;
      case "days":
  ...
  const estimatedWeightFromAge = useMemo(() => {
    if (tempWeight) return null;
    return estimateWeightFromAge(buildAgeString());
  }, [tempWeight, tempAgeValue, tempAgeUnit]);
  ...
  const handleAnswer = (
    ...
    setSession(prev =>
      answerPrimarySurvey(prev, question.id, answer, question, numVal, numVal2)
    );
    setNumberInput("");
    setNumberInput2("");
  };
  ...
  const handleUpdatePatientInfo = () => {
    const parsedWeight = tempWeight ? parseFloat(tempWeight) : null;
    const newWeight =
      parsedWeight !== null && !Number.isNaN(parsedWeight)
        ? parsedWeight
        : null;
    const newAge = buildAgeString();

    if (newWeight !== null || newAge !== null) {
      setSession(prev => updatePatientInfo(prev, newWeight, newAge));
      setDemographics({
        ...demographics,
        age: newAge || demographics.age,
        // keep weight empty if not manually entered; context auto-estimates from age
        weight: tempWeight || "",
      });
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <Sheet
        open={interventionPanelOpen}
        onOpenChange={setInterventionPanelOpen}
      >
        <SheetContent
          side="right"
          ...
          <SheetHeader className="pb-2">
            ...
            <SheetDescription className="text-muted-foreground">
              ...
              {session.fluidTracker.bolusCount > 0 && (
                <>
                  {" "}
                  &bull; {session.fluidTracker.bolusCount} bolus
                  {session.fluidTracker.bolusCount !== 1 ? "es" : ""}
                  {session.patientWeight
                    ? ` (${Math.round(session.fluidTracker.totalVolumePerKg)} mL/kg)`
                    : ` (${Math.round(session.fluidTracker.totalVolumeMl)} mL)`}
                </>
              )}
              {session.fluidTracker.isFluidRefractory && (
                <span className="text-red-400 font-semibold">
                  {" "}
                  &bull; FLUID-REFRACTORY
                </span>
              )}
            </SheetDescription>
          </SheetHeader>
        ...
        <DialogContent className="bg-background border-border">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              Update weight and age at any point. Drug doses recalculate
              automatically.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                <Input
                  ...
                  placeholder="e.g., 5"
                  value={tempAgeValue}
                  onChange={e => setTempAgeValue(e.target.value)}
                  className="col-span-2 bg-background text-foreground"
                />
                <select
                  value={tempAgeUnit}
                  onChange={e =>
                ...
                >
                  ...
                  <option value="gestation_weeks">gestation wks</option>
                </select>
              </div>
              <p className="text-xs text-muted-foreground mt-1">
                Enter age first. Weight auto-estimates from age if manual weight
                is not entered.
              </p>
            </div>
            ...
            <div>
              <label className="text-sm font-medium text-foreground mb-1 block">
                Weight (kg) — optional manual override
              </label>
              <Input
                type="number"
              ...
              />
              {!tempWeight && estimatedWeightFromAge !== null && (
                <p className="text-xs text-primary mt-1">
                  Auto-estimated weight: {estimatedWeightFromAge} kg
                </p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-1 font-mono text-xs">
              {session.events.map((evt, i) => {
                const elapsed = Math.floor(
                  (evt.timestamp - session.startTime) / 1000
                );
                const time = formatTime(elapsed);
                return (
                  <div
                    key={i}
                    className="flex gap-2 py-1 border-b border-border/30"
                  >
    ...
    <div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border">
      <div className="container max-w-2xl flex items-center gap-2 py-2">
        ...
        <div className="flex items-center gap-1.5 text-sm font-mono">
          ...
          <span
            className={
              timer.elapsed > 300 ? "text-amber-400" : "text-foreground"
            }
          >
    ...
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4">
      <div className="text-center mb-8">
        ...
        <p className="text-muted-foreground text-sm max-w-sm mx-auto">
          Systematic XABCDE assessment with real-time threat detection,
          weight-based dosing, and intervention tracking
        </p>
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="bg-accent/30 rounded-lg p-3 text-center">
              ...
              {weight ? (
                ...
                <p className="text-sm text-amber-400">Auto from age</p>
              )}
            </div>
          </div>
          {!weight && (
            <p className="text-xs text-amber-400/80 mt-2 text-center">
              Weight auto-calculates from age — enter measured weight anytime to
              override
            </p>
          )}
        </CardContent>
...
function QuickAssessmentScreen({
  onAnswer,
}: {
  onAnswer: (a: "sick" | "not_sick") => void;
}) {
  return (
...
function PrimarySurveyScreen({
  ...
  const letterOrder: ABCDELetter[] = session.isTrauma
    ? ["X", "A", "B", "C", "D", "E"]
    : ["A", "B", "C", "D", "E"];
  const currentIdx = letterOrder.indexOf(session.currentLetter);
  const progress =
    ((currentIdx + (questions.length - unanswered.length) / questions.length) /
      letterOrder.length) *
    100;
  ...
  return (
    <div className="py-6">
      ...
      <div
        className={`flex items-center gap-3 mb-6 p-4 rounded-xl ${letterConfig.bgColor}`}
      >
        <div className={letterConfig.color}>{letterConfig.icon}</div>
        <div>
          <h2 className={`text-xl font-bold ${letterConfig.color}`}>
            {letterConfig.label}
          </h2>
          <p className="text-xs text-muted-foreground">
            Question {questions.length - unanswered.length + 1} of{" "}
            {questions.length}
          </p>
        </div>
      </div>
          ...
          {question.inputType === "number_pair" &&
            question.numberPairConfig && (
              <NumberPairInput
                ...
                session={session}
                onSubmit={(v1, v2) => {
                  const interp = question.numberPairConfig!.interpret(
                    v1,
                    v2,
                    session
                  );
...
function NumberInput({
  ...
  const isValid =
    !isNaN(numVal) && numVal >= config.min && numVal <= config.max;
...
function InterventionScreen({
  ...
  return (
    <div className="py-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {criticalPending.length} critical action
            {criticalPending.length !== 1 ? "s" : ""} pending
          </p>
        </div>
      </div>
          ...
          <Card key={intervention.id} className="bg-card border-red-500/20">
            <CardContent className="pt-4 pb-4">
              <div className="flex items-start gap-3">
                <Badge
                  variant="outline"
                  className="shrink-0 text-xs border-red-500/30 text-red-400 bg-red-500/10"
                >
                  {threat.letter}
                </Badge>
                <div className="flex-1">
      ...
      {/* Actions */}
      <div className="space-y-3">
        <Button variant="outline" className="w-full py-4" onClick={onOpenPanel}>
          <ListChecks className="h-4 w-4 mr-2" />
          View All Interventions (
          {activeThreats.reduce((sum, t) => sum + t.interventions.length, 0)})
        </Button>
        <Button className="w-full py-4" onClick={onReturnToPrimary}>
    ...
    <div className="py-6">
      ...
      <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-6 text-center mb-6">
        ...
        <p className="text-sm text-red-300/80 mb-4">
          Push Hard &bull; Push Fast &bull; Minimize Interruptions
        </p>
        ...
        <div className="flex items-center justify-center gap-4 mb-4">
          <Badge
            variant="outline"
            className="text-sm border-red-500/30 text-red-400"
          >
            CPR Cycle: {Math.floor(timer.elapsed / 120) + 1}
          </Badge>
          <Badge
            variant="outline"
            className="text-sm border-amber-500/30 text-amber-400"
          >
      ...
      {arrestThreat && (
        <div className="space-y-3 mb-6">
          {arrestThreat.interventions.map(intervention => (
            <Card
              key={intervention.id}
              className={`bg-card ${intervention.status === "completed" ? "opacity-50" : "border-red-500/20"}`}
            >
    ...
    <div className="py-6 space-y-6">
      ...
      <Card className="bg-card border-border">
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-3">
            ...
            {session.vitalSigns.sbp !== undefined &&
              session.vitalSigns.dbp !== undefined && (
                <VitalBadge
                  label="BP"
              ...
              return (
                <div key={field}>
                  ...
                  <Input
                    ...
                    onChange={e =>
                      setSession(updateSAMPLE(session, field, e.target.value))
                    }
      ...
      {session.threats.length > 0 && (
        <Card className="bg-card border-border">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm text-foreground">
              Threats Identified ({session.threats.length})
            </CardTitle>
          </CardHeader>
            ...
            {session.threats.map(t => (
              <div key={t.id} className="flex items-center gap-2 text-sm">
                ...
                <span className="text-xs text-muted-foreground ml-auto">
                  {t.interventions.filter(i => i.status === "completed").length}
                  /{t.interventions.length} done
                </span>
              </div>
            ))}
          </CardContent>
...
function ThreatCard({
  ...
  setReassessmentMode: (
    v: { interventionId: string; checkIndex: number } | null
  ) => void;
  session: ResusSession;
  setSession: (s: ResusSession) => void;
}) {
  const completed = threat.interventions.filter(
  ...
  return (
    <Card
      className={`bg-card border-border ${threat.severity === "critical" ? "border-red-500/20" : ""}`}
    >
      <CardContent className="pt-3 pb-3">
        {/* Header */}
        <button
          ...
          <div className="mt-3 space-y-2 border-t border-border pt-3">
            {threat.interventions.map(intervention => (
              <div key={intervention.id}>
                  ...
                  <div className="flex-1">
                    ...
                    {intervention.status === "in_progress" && (
                      <div className="flex gap-1 mt-2">
                        ...
                        {intervention.reassessmentChecks &&
                          intervention.reassessmentChecks.length > 0 && (
                            <Button
                              size="sm"
                              variant="outline"
                              className="h-7 text-xs"
                ...
                {/* Reassessment Flow */}
                {reassessmentMode?.interventionId === intervention.id &&
                  intervention.reassessmentChecks && (
                    <ReassessmentFlow
