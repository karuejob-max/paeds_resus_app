    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <header className="flex flex-col gap-4 rounded-2xl border bg-background p-5 shadow-sm md:flex-row md:items-start md:justify-between md:p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-medium text-primary">
              Paeds Resus · Global Admin
            </p>
            <h1 className="mt-1 text-2xl font-semibold tracking-tight">
              Platform overview
            </h1>
            <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">
              Start with the work area that matches the decision you need to
              make. Use Access grants for approved free or discounted access,
              and switch to Institution Workspace for tenant-scoped
              administration.
            </p>
          </div>
        </div>
        <Button
          ...
          className="shrink-0"
        >
          Switch to Institution Workspace{" "}
      ...
      <section
        ...
        aria-label="Priority admin actions"
      >
        <PriorityCard
          title="Access grants"
      ...
      <section aria-labelledby="work-areas-heading" className="space-y-3">
        <div>
          <h2 id="work-areas-heading" className="text-lg font-semibold">
            Work areas
          </h2>
          <p className="text-sm text-muted-foreground">
            Every destination is grouped by the type of admin work it supports.
          </p>
        </div>
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {adminNavigationGroups
            .filter(
              group =>
                group.label !== "Overview" && group.label !== "Maintenance"
            )
            .map(group => {
              ...
              return (
                <Card key={group.label} className="border-border/80 shadow-sm">
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      {group.items.length} destination
                      {group.items.length === 1 ? "" : "s"} available
                    </CardDescription>
                  </CardHeader>
    ...
    <Card className="border-primary/15 bg-background shadow-sm transition-colors hover:border-primary/40">
      <CardHeader>
        ...
        <Button
          type="button"
          variant="link"
          className="h-auto justify-start px-0 pt-3"
...
function MaintenanceCard() {
  ...
  return (
    <section
      ...
      className="rounded-xl border border-slate-200 bg-background p-4 shadow-sm md:p-5"
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Rare, privileged technical actions. Course image URL migration is
              safe to run repeatedly but should not be treated as routine admin
              work.
            </p>
          </div>
        </div>
