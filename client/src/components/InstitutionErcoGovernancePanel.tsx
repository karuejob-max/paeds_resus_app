...
export function InstitutionErcoGovernancePanel({ institutionId }: InstitutionErcoGovernancePanelProps) {
  ...
  const [mismatchTargetUserId, setMismatchTargetUserId] = useState<number | null>(null);
  const [mismatchReason, setMismatchReason] = useState("");
  const [confirmedMismatchUserIds, setConfirmedMismatchUserIds] = useState<number[]>([]);
  ...
  const departmentNurseCandidates = useMemo(
    () => (departmentCandidates ?? []).filter((candidate) => candidate.assignable),
    [departmentCandidates],
  );
  const pendingDepartmentNurseCandidates = useMemo(
    () => (departmentCandidates ?? []).filter((candidate) => !candidate.assignable),
    [departmentCandidates],
  );
  ...
  const reportMismatchMutation = trpc.institution.reportDepartmentMismatch.useMutation({
    onSuccess: (result) => {
      if (mismatchTargetUserId != null) {
        setConfirmedMismatchUserIds((current) => current.includes(mismatchTargetUserId) ? current : [...current, mismatchTargetUserId]);
      }
      toast.success(result.duplicate ? "This department conflict is already with the institutional administrators." : "Department conflict confirmed and sent to institutional administrators.");
      setMismatchTargetUserId(null);
      setMismatchReason("");
    },
    onError: (error) => toast.error(error.message || "Could not send the mismatch alert."),
  });
      ...
      <CardContent className="space-y-5">
        ...
        {selectedDepartment && (
          <div ref={configurationRef} id="iers-erco-configuration" className="scroll-mt-24 grid gap-4 rounded-lg border bg-background p-4 md:grid-cols-2">
            ...
            <label className="space-y-1 text-sm"><span className="font-medium">Emergency Readiness Coordinator (ERCo)</span><Select value={coordinatorUserId} onValueChange={setCoordinatorUserId} disabled={departmentCandidatesLoading}><SelectTrigger aria-label={`Select ERCo for ${selectedDepartment.departmentName}`}><SelectValue placeholder={departmentCandidatesLoading ? "Loading department Staff/RN practitioners…" : "Select department nurse"} /></SelectTrigger><SelectContent>{departmentNurseCandidates.map((staff) => <SelectItem key={staff.userId} value={String(staff.userId)}><span className="flex items-center gap-2"><UserRound className="h-3.5 w-3.5" />{staff.staffName} ({staff.staffRole})</span></SelectItem>)}{pendingDepartmentNurseCandidates.map((staff) => <SelectItem key={`pending-${staff.id}`} value={`pending-${staff.id}`} disabled>{staff.staffName} · {staff.departmentMismatch ? `current department: ${staff.currentDepartment ?? "not recorded"}` : "account link or active membership required"}</SelectItem>)}</SelectContent></Select>{!departmentCandidatesLoading && departmentNurseCandidates.length === 0 && <span className="block space-y-2 text-xs text-amber-700 dark:text-amber-300"><span className="block">{pendingDepartmentNurseCandidates.length > 0 ? `${pendingDepartmentNurseCandidates.length} registered Staff/RN candidate(s) are visible but need an accepted institution link before assignment.` : "No active linked non-student Staff/RN practitioner is registered with this department yet."}</span><Button asChild type="button" size="sm" variant="outline" className="mt-1"><a href="/institution?section=administration&adminTab=institution&peopleTab=people_access">Open People &amp; roles</a></Button></span>}
              {pendingDepartmentNurseCandidates.some((staff) => staff.departmentMismatch && staff.userId != null) && <div className="mt-2 space-y-2 rounded-md border border-amber-300 bg-amber-50 p-2.5 text-xs dark:border-amber-900 dark:bg-amber-950/20"><p className="font-medium text-amber-900 dark:text-amber-100">Department membership mismatch</p><p className="text-amber-800 dark:text-amber-200">A nurse has CPD/profile evidence for this department, but the current institutional roster places them elsewhere. Confirm the conflict for the institutional administrator to reallocate or retire; this does not assign the nurse.</p>{pendingDepartmentNurseCandidates.filter((staff) => staff.departmentMismatch && staff.userId != null).map((staff) => <div key={`mismatch-${staff.userId}`} className="rounded border bg-background p-2"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="font-medium">{staff.staffName} <span className="font-normal text-muted-foreground">(current: {staff.currentDepartment ?? "not recorded"})</span></span><Button type="button" size="sm" variant="outline" disabled={confirmedMismatchUserIds.includes(staff.userId ?? -1) || reportMismatchMutation.isPending} onClick={() => { setMismatchTargetUserId(staff.userId); setMismatchReason("ERCo confirmed the department membership mismatch for institutional administrator review."); }}>{confirmedMismatchUserIds.includes(staff.userId ?? -1) ? "Conflict confirmed" : "Confirm conflict"}</Button></div>{mismatchTargetUserId === staff.userId && <div className="mt-2 space-y-2"><Input value={mismatchReason} onChange={(event) => setMismatchReason(event.target.value)} aria-label="Department conflict confirmation reason" placeholder="Confirmation reason (at least 10 characters)" /><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" size="sm" disabled={mismatchReason.trim().length < 10 || reportMismatchMutation.isPending} onClick={() => selectedDepartmentId != null && reportMismatchMutation.mutate({ institutionId, departmentId: selectedDepartmentId, providerUserId: staff.userId as number, reason: mismatchReason.trim() })}>{reportMismatchMutation.isPending ? "Confirming…" : "Confirm conflict"}</Button><Button type="button" size="sm" variant="ghost" onClick={() => { setMismatchTargetUserId(null); setMismatchReason(""); }}>Cancel</Button></div></div>}</div>)}</div>}
              </label>
            <label className="space-y-1 text-sm"><span className="font-medium">Assistant ERCo <span className="font-normal text-muted-foreground">(optional)</span></span><Select value={backupUserId} onValueChange={setBackupUserId} disabled={departmentCandidatesLoading}><SelectTrigger aria-label={`Select assistant ERCo for ${selectedDepartment.departmentName}`}><SelectValue placeholder={departmentCandidatesLoading ? "Loading department Staff/RN practitioners…" : "Select assistant nurse"} /></SelectTrigger><SelectContent><SelectItem value="none">No Assistant ERCo</SelectItem>{departmentNurseCandidates.filter((staff) => String(staff.userId) !== coordinatorUserId).map((staff) => <SelectItem key={staff.userId} value={String(staff.userId)}><span className="flex items-center gap-2"><UsersRound className="h-3.5 w-3.5" />{staff.staffName} ({staff.staffRole})</span></SelectItem>)}{pendingDepartmentNurseCandidates.filter((staff) => String(staff.userId) !== coordinatorUserId).map((staff) => <SelectItem key={`pending-assistant-${staff.id}`} value={`pending-assistant-${staff.id}`} disabled>{staff.staffName} · account link or active membership required</SelectItem>)}</SelectContent></Select></label>
