...
async function createActivationTeamSnapshot(db: DbClient, input: { activationEventId: number; team: typeof iersShiftTeams.$inferSelect; assignments: Array<typeof iersShiftRoleAssignments.$inferSelect> }) {
  ...
  const orderedAssignments = [...input.assignments].sort((left, right) => {
    const acceptedFirst = Number(right.assignmentStatus === "accepted") - Number(left.assignmentStatus === "accepted");
    return acceptedFirst || right.id - left.id;
  });
  for (const assignment of orderedAssignments) {
    if (!activeProviderIds.has(assignment.providerUserId) || assignment.departmentId == null) continue;
    const snapshotKey = `${assignment.providerUserId}:${assignment.roleKey}`;
...
export const iersRouter = router({
  ...
  triggerActivation: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // This nonce only correlates the insert. It must not remain in caseQrNonce,
      // which is reserved for the real case QR generated by the first responder.
      await db.update(iersActivationEvents)
        .set({ caseQrNonce: null, updatedAt: new Date() })
        .where(and(
          eq(iersActivationEvents.id, activationEventId),
          eq(iersActivationEvents.caseQrNonce, activationCorrelationNonce),
          isNull(iersActivationEvents.caseQrGeneratedByUserId),
        ));
  ...
  generateCaseQr: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!currentQrState?.caseQrNonce) {
        await db.update(iersActivationEvents).set({ caseQrNonce: nonce, caseQrGeneratedByUserId: ctx.user.id, caseQrGeneratedAt: now, updatedAt: now }).where(and(eq(iersActivationEvents.id, input.activationEventId), isNull(iersActivationEvents.caseQrNonce)));
        const [persistedQrState] = await db.select({ caseQrNonce: iersActivationEvents.caseQrNonce, caseQrGeneratedByUserId: iersActivationEvents.caseQrGeneratedByUserId }).from(iersActivationEvents).where(eq(iersActivationEvents.id, input.activationEventId)).limit(1);
        generatedNewQr = persistedQrState?.caseQrNonce === nonce && persistedQrState.caseQrGeneratedByUserId === ctx.user.id;
      }
      if (generatedNewQr) {
