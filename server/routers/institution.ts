...
import { getProviderScorecard, getFacilityMedianQiCount } from "../services/provider-performance.service";
...
export const institutionRouter = router({
   ...
   * stated limitations, especially the CPD email-matching caveat and the
   * assumed 2-year Life Support validity window).
   */
  ...
  getStaffPerformanceRoster: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (!linkedFacility) {
        return { lastDays, roster: [] as Awaited<ReturnType<typeof getProviderScorecard>>[] };
      }
      ...
      return { lastDays, roster: roster.filter((r): r is NonNullable<typeof r> => r != null) };
    }),
