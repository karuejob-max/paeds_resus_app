...
### Payment terms (CEO decision, 2026-07-19; ToU restructured 2026-07-21)
- All payments under this program, including instalments, are **non-refundable** — Terms of Use §6.4 (`docs/legal/TERMS_OF_USE_FULL.md`), synced in `client/src/legal/terms-of-use.ts` §9. **§6.4 is now split into §6.4.1 (IERP) and §6.4.2 (Nurse Cohort Program)**, each with its own clearly labeled rules, rather than one paragraph covering both tracks with conditional language — matches the `shared/program-identity.ts` split (see below) so the legal document and the in-app program badge/preview say the same thing. `termsOfUse` bumped to 1.2.0 in `shared/legal-versions.ts` — re-triggers the consent gate for every existing user. **Fixed while bumping:** the nurse instalment-pace rule (§ below) had been enforced in code since INST-17 but was never actually documented in the Terms of Use until this pass — a real gap, not just a rewording.
