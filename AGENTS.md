...
### Program identity (CEO decision, 2026-07-21)
Interns and nurses go through visually identical dashboard/course screens with different payment rules operating invisibly in the background — a real confusion risk once IERP (intern) and the Nurse Cohort Program are marketed as distinct audiences. `shared/program-identity.ts`'s `getProgramIdentity(designation)` is the single source of truth for "which program is this person on, and what actually applies to them" — **derived from the existing `designation` field, not a separate stored value**, deliberately avoiding a second field that could drift out of sync with the one that actually drives eligibility. Consumed by `courses.ts`'s `getPhaseSummary` (adds `programIdentity` to its response) and by `LearnerDashboard.tsx`'s new `ProgramIdentityBadge` (persistent "You're on: IERP / Nurse Cohort Program" badge) and the `DesignationDeclarationCard`'s live rules preview (shown the moment a designation is selected, before submit — so a learner sees their actual rules right at the point of committing to a designation, not buried in a document afterward). One shared function, both server and client import it directly — not duplicated logic in each place. Tested in `shared/program-identity.test.ts`.
