...
The current admin NERP screen verifies Phase 2 and Phase 3 only through a `nerp_offer_enrollments` ledger. This means an administrator cannot create a verified external-completion case for a person who completed training outside the NERP offer pathway or who has not registered. The existing verification table is correctly phase-specific, but it is structurally tied to the offer ledger. The external-case model now supports two explicit categories: `nerp_nurse` for nurses being reviewed as NERP completions and `non_nurse_external` for non-nurses who completed the training outside the platform pathways. Non-nurse cases require a recorded cadre, may link to an existing account only on an exact normalized email match, and never enter the NERP nurse campaign audience or grant IERS access.
...
The NERP change adds the candidate category and cadre to the existing external-completion case model, leaving offer payment rules unchanged. The CPD change now resolves active institutional members from signed-up membership plus roster/profile data, filters them by assigned department, uses the same directory for People and targets and CPD participants, derives the lead presenter cadre/name/department server-side, and rejects non-member lead or co-presenter identities. Audience scope remains structured; an other-cadre audience must be selected from cadres represented in the scoped member directory. Legacy event creation remains compatible at the input boundary but now also requires a selected active institution member. IERS competency and NERP programme rules are not changed except for the additive external verification and suppression controls explicitly requested here.
