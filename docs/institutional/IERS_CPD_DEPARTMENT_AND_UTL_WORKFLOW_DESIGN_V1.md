...
## 7. Implementation addendum — 2026-08-23

The first implementation slice keeps the existing `facility_departments` registry as the only institution-scoped canonical identity. Account administrators can add or reactivate a named local department from Administration, with an explicit reason and append-only audit event. New CPD registrations and linked provider profiles receive the active confirmed institution department list before the shared catalog fallback, reducing unnecessary `Other` usage without removing the genuine custom exception.

Administration also exposes an institution-scoped traceability report for exact `Other` submissions and unresolved custom labels. It shows the captured attendee name, email, phone, cadre, event, date, raw recorded department text, attendance type, and canonical-link status. Historical CPD text remains unchanged. A linked row can still appear in the report when its raw text was exactly `Other`, which preserves the accountability trail.

The Institution Workspace now persists CPD and IERS subtab choices in URL parameters and keeps compact sticky tab strips available during long-page scrolling. IERS workforce is divided into departments and poles, ERCo governance, and shift staffing. Step 3 no longer implies that the first matching provider will work the month. Every active confirmed pole-required department on the selected pole is listed; an authorized administrator or accepted department ERCo can assign the ERCo where permitted, select an optional monthly source provider, add a manual nurse candidate for account linking, and select the actual nurse for each dated shift. A monthly plan may deliberately remain unassigned. Provider-owned acceptance remains required for every dated duty.

This workflow is request-driven and does not create a background scheduler. It does not alter the clinical emergency flow, create patient data, or infer competence from a profile, membership, product role, or roster row.
