...
**Routes (from `App.tsx`):** `/institutional`, `/hospital-admin-dashboard`, `/institutional-onboarding` (alias: `/institution-onboarding` → redirect), and legacy `/institutional-portal` redirect compatibility.
