...
- [ ] **`pnpm run check`** (`tsc --noEmit`) passes. Prefer **`pnpm run ci:gate`** (includes `verify:ci`) before merge to `main` — [PRE_MERGE_CHECKLIST.md](./PRE_MERGE_CHECKLIST.md). Run **`pnpm run test:clinical`** when touching Resus/clinical engines.
