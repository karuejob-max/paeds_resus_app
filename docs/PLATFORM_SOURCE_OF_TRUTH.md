...
An authorized ERTL, UTL, ERCo coordinator, or institution administrator may call off an active activation from the linked provider case panel by choosing response no longer required (`cancelled`) or false alarm (`false_alarm`) and entering a reason. Terminal transitions require a reason, append the actor and transition to the activation timeline, preserve the activation record for audit, and stop further response actions. The server sends a best-effort closure message to the exact notification tag so service workers dismiss persistent browser alerts; foreground clients also close the matching alert when polling observes the terminal status. If push delivery is unavailable, database status and in-app polling remain authoritative. Calling off an event is distinct from a completed clinical close/debrief (`closed`), which still requires the appropriate preceding lifecycle and debrief finding.
...
**NERP campaign governance:** campaign preview excludes verified external completers and durable exact-match suppressions by normalized email or exact full name. Suppression-only records may represent an unregistered person and are never treated as sendable recipients. Suppression changes are append-only and auditable. Campaign delivery remains disabled in this release; preview and exclusion reasoning are the only permitted operations.
