...
The dated ERTL / Scene Commander is visible on the provider’s pole-scoped team. When a valid weekly rotation names an ERTL, that eligible linked Staff/RN provider is projected for the exact dated shift. When the rotation has no valid nominated provider, the explicitly staffed UTL from the rotation department may be materialized as the separate dated `ertl` scope for that shift; this is a projection of institution-authored staffing, not automatic provider creation or silent staffing. A provider may therefore hold separate `utl` and `ertl` rows for one shift when serving as Scene Commander, and each scope retains its own explicit acceptance state.
