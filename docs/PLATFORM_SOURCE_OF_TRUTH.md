...
Provider performance is part of this minimal contract. Today must load only the current dated team and urgent activation state immediately; non-urgent duty and readiness summaries may load after the first paint. My Shift must meet the current team first, defer heavier institutional responsibility content until requested, and expose future teams through deliberate expansion. The global non-miss activation observer owns urgent polling; individual pages must not start duplicate activation timers. Read-repair may self-heal a UTL-backed team, but it must skip teams that already have a live ERTL projection and must exclude ended, expired, or superseded assignments from the active provider payload. These optimizations must never weaken exact institution/pole scope, active membership, explicit assignment, acceptance, or activation authorization.
...
- **Applied / enrolled:** A row in **`enrollments`** = one application. Authoritative fields include `createdAt`, `programType` (`bls` \| `acls` \| `pals` \| `fellowship` \| `instructor` \| `paeds_resus_ils`), `paymentStatus`, etc. There is **no** strict enforced state machine in code yet; metrics are **counts by date** unless we add a formal funnel later.
- **Certified:** A row in **`certificates`** = one certificate. Authoritative fields include `issueDate`, `programType`.
- **Certification Strategy (Incentives):** To maintain high learner motivation and clinical rigor, certificates are issued for **every** successfully completed micro-course, every AHA course (BLS, ACLS, PALS), the **Institutional Life Support Training Program** after its cognitive and practical gates, and finally the **Fellowship Diploma** upon graduation. The Institutional Life Support certificate is a **Paeds Resus competency certificate**, not an AHA certificate. Certificates must be verifiable and downloadable on-demand from the provider dashboard.
...
| **Professional competence** | Knowledge gaps, certification, behaviour change | Fellowship + AHA Courses + Institutional Life Support + Micro-courses + IERP + NERP |
...
Provider identity (users + providerProfiles)
    ...
    │          Universal Paeds Resus completion certificates remain in the
    │          shared certificate ledger and are separately identifiable from
    │          official AHA certificates.
    │
    ├── Institutional Life Support state (enrollments + institutionalTrainingOrders + institutionalTrainingOrderProviders + ilsCredentialRequests)
    │       └── feeds Paeds Resus competency certification, institution-paid provider cohorts, and separately reviewed AHA credentialing requests
    │
    ...
    │
    ├── NERP external completion and campaign governance
    │       └── admin-reviewed phase evidence for NERP nurses and non-nurse outside-pathway
    │          completion cases, with exact suppression and audit records; non-nurse cases
    │          never enter the NERP nurse campaign audience or grant IERS access
    │
    ├── Institutional member directory and CPD participation
    │       └── active institution memberships enriched by roster/profile department data;
    │          People & targets and CPD presenter/audience selection use the same scoped
    │          directory, while server-side session creation derives participant identity
    │          and cadre rather than trusting free text
    │
    ├── Professional credential ledger (professionalCredentials)
    │       └── feeds provider verification + institution compliance reporting
    │          (Paeds Resus-derived Life Support records are read-only projections;
    │           external AHA evidence remains a separate verified source)
    │
...
- NERP administration also supports reviewed external-completion cases for people who completed training outside the platform. Cases are explicitly categorized as `nerp_nurse` or `non_nurse_external`; non-nurse cases require a cadre, may link only on an exact normalized email match, and remain outside the NERP nurse campaign audience and IERS permissions.
- The individual, self-funded, non-cohort route is named the **Paeds Resus Open Enrolment Pathway**. It is a pathway, not a subsidized programme, and follows the same verified Paeds Resus completion and provider-certificate standards without inheriting IERP or NERP payment rules.
- A verified Phase 2 completion issues one **Paeds Resus Phase 2 — Online Simulations** certificate and unlocks the Phase 3 hands-on pathway. A verified Phase 3 completion issues the corresponding **Paeds Resus Certified BLS, ACLS, PALS, or NRP Provider** certificate. These are Paeds Resus completion credentials, not replacements for official AHA certificates.
- Certificates are issued idempotently from authoritative completion sources, are downloadable through the existing owner-authenticated certificate route, and can be publicly verified by certificate number plus recipient name. The `professionalCredentials` ledger receives read-only derived projections.
...
- **Feeds:** Facility-level analytics, MOH reporting, procurement intelligence, ERT activation metrics, criterion-level readiness evidence, drill/debrief learning, and owned QI actions
- **Receives from:** Care Signal and Code Signal (facility QI signals), ResusGPS (case/adoption signals), Course completions (staff training status), provider memberships (named responsibilities), activation timelines, equipment audits, drills, and provider-submitted evidence
- **Integration shipped:** Providers can trigger/acknowledge/respond to activations, sign off assigned shifts, submit evidence, report gaps, progress actions, join drills, and feed QI signals from the provider platform. Institution leaders review evidence, verify action closure, and manage the 30/60/90-day implementation plan.
- **Provider duty governance shipped:** Each institution department has exactly one current standing Emergency Response Coordinator (ERCo) record, with an optional backup. ERCo, backup, named weekly ERTL, and shift UTL responsibilities are dated, identity-bound, explicitly accepted or declined, and auditable; membership, generic roster placement, notification, or administrator assignment alone never proves accepted duty or competency. Shift-readiness sign-off is blocked until the assigned provider accepts the dated duty. Assignment changes reset acceptance and readiness evidence.
- **Activation case-link shipped:** My Shift selects the exact current local shift interval first; future dated teams remain available only through explicit selection. Weekly ERTL / Scene Commander status is derived from the accepted UTL of the pole’s leading department, with no manual ERTL staffing or automatic role acceptance. A confirmed current-team activation snapshots the dated team, pole, provider roles, and assignment states; it notifies only that scoped team, keeps per-provider receipt/response states separate from the global event state, carries critical readiness gaps as visible activation resources, and records self, witnessed, or QR-scanned arrival evidence. The first responding provider generates a signed opaque case QR from ResusGPS. Later authenticated assigned responders use the in-app scanner to join the same case and record arrival; the QR contains only activation ID and nonce, never patient identifiers or unrestricted roster data. ResusGPS, named role reports, and anonymous Care/Code Signal remain separate but may reference the same activation event. No QR join or activation is a substitute for provider identity, active membership, dated assignment, explicit acceptance, or clinical documentation.
- **Urgent notification layer shipped:** The provider Today workspace offers deliberate device setup for foreground audio/haptic alerts and browser notification permission. The global activation alert triggers once per pending responder state and retains the visual in-app modal and five-second polling fallback. When `VITE_ENABLE_SW=true` and server VAPID variables are configured, the active `/sw.js` service worker receives a privacy-preserving payload containing only activation location, optional bed, department, needed resources, and a ResusGPS route; durable subscription and push-service attempt records are stored in `iers_push_subscriptions` and `iers_push_delivery_log`. A `sent` push status means accepted by the push service, not confirmed device receipt; provider receipt/response mutations remain the authoritative acknowledgement evidence. If configuration, permission, connectivity, background execution, or device sound is unavailable, the platform must not claim guaranteed delivery and must retain the in-app visual fallback.
- **Credential and institutional accountability layer shipped:** `professionalCredentials` is the structured source for regulatory licences, external AHA BLS/ACLS/PALS/NRP evidence, and read-only Paeds Resus cognitive, simulation, and provider projections derived from authoritative enrollment/certificate records.
- **Learning/CPD participant identity:** department-scoped People & targets and CPD session creation resolve active institution members from membership, roster, and profile sources. Lead presenters, cadres, co-presenters, and other-cadre audiences are directory-driven; server-side writes reject arbitrary non-member presenter identities. Legacy CPD entry remains compatible only as a lookup path, not as a trusted free-text identity source. `professionalCredentialReminderEvents` deduplicates reminders at three, two, and one calendar month before expiry and weekly after expiry. Institutional compliance projections expose only the minimum fields required by the viewer's institution, product role, account scope, or department appointment. `institutionDepartmentHeads` supports one active Departmental Head per canonical department with an append-only appointment history; Departmental Head and Education Coordinator views are department-scoped and never grant IERS duties or product permissions by implication. General institution members receive safe aggregate context only; personal contacts, evidence files, individual performance, and identifiable CPD history remain restricted to authorised roles.
- **Safety rule:** Evidence-derived scores are internal readiness measures. Certification review requires accepted critical evidence; no self-scored total can bypass missing proof.
- **Canonical implementation docs:** [IERS_BUILD_BLUEPRINT_V1.md](./institutional/IERS_BUILD_BLUEPRINT_V1.md), [IERS_OPERATING_GUIDE_V1.md](./institutional/IERS_OPERATING_GUIDE_V1.md), [IERS_PROVIDER_INTEGRATION_AND_INDIVIDUAL_PORTAL_ARCHITECTURE_V1.md](./institutional/IERS_PROVIDER_INTEGRATION_AND_INDIVIDUAL_PORTAL_ARCHITECTURE_V1.md)
...
| [NERP_CPD_SIMPLIFICATION_AUDIT.md](./NERP_CPD_SIMPLIFICATION_AUDIT.md) | Active — Engineering/audit | External NERP completion review, precise campaign suppression, department-scoped CPD access, and simplified Learning workspace information architecture |
...
| [institutional/IERS_BUILD_BLUEPRINT_V1.md](./institutional/IERS_BUILD_BLUEPRINT_V1.md) | Active — Engineering contract | IERS build contract: provider responsibility, institution roles, activation safety states, evidence rules, and release gates |
| [institutional/IERS_OPERATING_GUIDE_V1.md](./institutional/IERS_OPERATING_GUIDE_V1.md) | Active — Operational | IERS operating loop, role responsibilities, evidence-derived scoring, production migration order, and pilot acceptance tests |
| [institutional/IERS_PROVIDER_INTEGRATION_AND_INDIVIDUAL_PORTAL_ARCHITECTURE_V1.md](./institutional/IERS_PROVIDER_INTEGRATION_AND_INDIVIDUAL_PORTAL_ARCHITECTURE_V1.md) | Active — Engineering contract | Provider-integrated IERS ownership, exactly one ERCo per department, dated duty acceptance, provider portal lanes, activation/debrief evidence, release gates, and pilot pre-mortem |
| [institutional/IERS_CPD_DEPARTMENT_AND_UTL_WORKFLOW_DESIGN_V2.md](./institutional/IERS_CPD_DEPARTMENT_AND_UTL_WORKFLOW_DESIGN_V2.md) | Active — Engineering/operational contract | Per-attendee `Other` resolution, People & profile navigation, ERCo governance versus dated duty, explicit UTL staffing, and deterministic pole-order ERTL rotation |
...
### 24.2a Institutional onboarding and Institutional Life Support

Institution onboarding is category-led and country-neutral. The organization category is selected from healthcare facilities, teaching/referral facilities, health-professional education institutions, continuing professional-development providers, pre-hospital/emergency services, public-health programmes, professional associations/NGOs, healthcare networks/systems, research/academic health organizations, or another healthcare/public-health organization. Faith-based and mission ownership is captured separately from service category rather than being treated as a country-specific category.

When a healthcare or teaching/referral facility is selected, onboarding captures ownership and the closest care classification: **Primary care — Level 1, 2, 3, or 4; Secondary care — Level 5; Tertiary care — Level 6; Quaternary care; or another national classification/not sure with the local designation**. The parent care tier and local designation keep the field useful outside Kenya; no country is forced into the Kenya Level 1–6 model.

The **Institutional Life Support Training Program** is a first-class **institution-paid cohort** learning product. The base price is **KES 10,000 per provider**. Institution coordinators select linked Paeds Resus provider accounts, place one bulk order, and pay for the cohort; an individual provider does not purchase the base programme directly. A provider can access the programme only after institution assignment and confirmed cohort payment. The programme uses online modules, knowledge checks, practical assessment, and Paeds Resus competency certification. It does not issue an AHA certificate. After Paeds Resus certification, a learner may request BLS credentialing for **KES 7,500** or ACLS credentialing for **KES 10,000**, provided the request and payment are completed within **90 days** of certificate issue. No additional training is charged during that window, but the request remains subject to AHA credentialing review. After the window, a new full training enrolment applies at **KES 10,000 for BLS** or **KES 20,000 for ACLS**. An unpaid, unstarted ILS enrollment can be cancelled reversibly: the enrollment and any pending payment row are retained for audit, active access is removed, and delayed payment callbacks are ignored.
...
**Canonical department identity:** `facility_departments` is the institution-scoped source of truth for departments and their optional facility poles. During onboarding, the institutional admin confirms the complete department list; later updates can rename, add, or deactivate rows while preserving canonical IDs. IERS leads assign each confirmed department to a pole, and ERCo-led monthly UTL rota preparation uses the same department-to-pole registry. Linked provider records resolve their preset department to `facilityDepartmentId`; the monthly source row records provider and provenance, while each dated shift remains an explicit provider-accepted duty. Weekly ERTL selection uses the canonical department and automatically refreshes monthly-derived shift flags when the weekly department changes. CPD registration must offer the same institution’s configured department list and persist `facilityDepartmentId` on new CPD attendance and linked institutional staff rows. Historical department text remains for reporting compatibility; migration `0113` backfills only exact case-insensitive institution-scoped matches and migration `0114` adds confirmation state, monthly UTL provenance, and strict verifier coverage. CPD and IERS remain independently gated products, but they intentionally share department identity where both are active. The preferred label source is the shared preset catalog in `shared/clinical-departments.ts`, already consumed by profile and CPD selectors; an IERS setup user may choose `Other` only for a genuine department absent from that catalog, and custom text must remain explicitly identifiable rather than silently treated as a preset.

**CPD reconciliation and IERS pole eligibility:** migration `0115` adds the fail-closed `facility_departments.requires_pole` decision. It defaults to `false`; catalog membership, CPD attendance, provider cadre, and custom-label frequency never infer IERS operational eligibility. Only a confirmed active department explicitly marked `requires_pole=true` and still lacking `pole_id` produces an IERS Lead missing-pole alert. Pharmacy and other CPD/reporting-only departments therefore remain valid without a pole. Account administrators manually review durable reconciliation rows in `institution_department_reconciliations`, may map a historic custom label to an explicitly selected local department, and may backfill only nullable `cpdAttendees.facilityDepartmentId`; raw `cpdAttendees.department` text is immutable for reporting. Mapping, review, backfill, and eligibility decisions are recorded in append-only `institution_department_audit_events`. Administration can now add or reactivate a named local department through the same registry rather than forcing future users into `Other`; the portal also exposes institution-scoped attendee name, email, phone, cadre, event, recorded label, and canonical-link status for exact `Other` or unresolved custom submissions. The full reconciliation contract is [IERS_CPD_DEPARTMENT_RECONCILIATION_DESIGN_V1.md](./institutional/IERS_CPD_DEPARTMENT_RECONCILIATION_DESIGN_V1.md). Literal `Other` submissions are not one department group: each attendee is reviewed separately and may resolve to a different canonical department. Unresolved custom labels remain a separate pattern-review queue. The corrected governance and rota semantics are recorded in [IERS_CPD_DEPARTMENT_AND_UTL_WORKFLOW_DESIGN_V2.md](./institutional/IERS_CPD_DEPARTMENT_AND_UTL_WORKFLOW_DESIGN_V2.md).

**CPD facility relationship and global facility history:** every authenticated CPD registration records whether the host is the provider's `permanent_facility` or a `locum_outreach` site. CPD attendance is the default facility-link signal: a signed-in attendee is automatically associated with the host institution as a general staff member, whether the attendance is permanent or outreach/locum. Permanent attendance may update the provider's primary facility; locum/outreach attendance is retained as a secondary facility history and does not replace the primary facility. This association never creates an IERS product role, dated duty, team publication, or readiness sign-off. Existing suspended, ended, rejected, removed, or conflicting relationships remain administrator-controlled and cannot be silently reactivated by later CPD attendance. The global provider profile exposes all CPD-linked facilities with their permanent/locum relationship and membership state. Institution administrators may explicitly unlink a person (ending access while retaining CPD and audit history), retire them from the institutional register, or reallocate their current canonical department. If an ERCo identifies a provider whose CPD/profile evidence points to the ERCo's department but whose current roster department or membership is not current, the ERCo may raise a reasoned mismatch alert; the institution administrator receives an auditable notification and must reallocate or retire the person. No such alert or reallocation automatically assigns an IERS duty.

**Administration People & roles:** the product-role catalog exposes the `IERS Lead`, response operator, reviewer, governance, and viewer roles. The read-only duty ledger separately displays exactly one current ERCo per department, optional Assistant ERCo, dated ERTL rotations, and dated UTL shifts with assignment, acceptance, decline, and readiness states. Administration also exposes account-admin-only CPD department reconciliation, canonical department creation/reactivation, and pole-eligibility controls; IERS Leads may view live missing-pole conditions and allocate poles but cannot rewrite CPD reconciliation. An ERCo role is a standing governance appointment, not day-to-day response coverage; an optional Assistant ERCo supports continuity only. A product role or governance responsibility never substitutes for provider-owned acceptance, competency evidence, or emergency dispatch proof.

**Compact navigation and UTL staffing:** the Institution Workspace persists CPD, IERS, and Administration → People & profile subtab choices in URL parameters and keeps compact sticky tab strips available while long panels scroll, so staff can deep-link to sessions, analytics, exports, workforce setup, ERCo governance, or shift staffing. Step 3 is not an automatic staffing decision: every active, confirmed, pole-required department assigned to the selected pole appears in Shift staffing. The IERS Lead or the accepted ERCo can assign/reassign an ERCo directly from that workflow where permitted, choose an optional monthly nurse source, add a manual nurse candidate for account linking, and then select the actual on-shift nurse for each dated shift. A monthly source row may remain intentionally unassigned; it never silently chooses the first provider, and every dated provider duty still requires explicit acceptance.

**Public IERS orientation:** `/iers/orientation` is the unauthenticated, concise public guide for providers, ERCos, IERS Leads, institution administrators, reviewers, and prospective clients. It explains the IERS operating loop, role boundaries, safety scope, reporting separation, and first-day actions. It is linked from the Individual provider dashboard and Institution Workspace header. Detailed client policy remains in `docs/institutional/IERS_CLIENT_ONBOARDING_AND_OPERATING_MANUAL_V1.md`, while local facility decisions belong in `IERS_FACILITY_ONBOARDING_APPENDIX_TEMPLATE.md`.

**NERP external completion verification:** NERP verification supports an administrator-reviewed external-completion case when a nurse completed the Nurses Emergency Readiness Program outside the platform pathway. The case is independent of the NERP offer/payment ledger, stores the external provider and certificate reference, and requires separate Phase 2 and Phase 3 decisions with reviewer notes before the case is considered externally verified. A normalized exact email can link an existing Paeds Resus account; an unregistered person remains a reviewable case without account creation. External verification is evidence of training status, not an IERS role, duty, readiness sign-off, or substitute for local competency governance.

**NERP campaign governance:** campaign preview excludes verified external completers and durable exact-match suppressions by normalized email or exact full name. Suppression-only records may represent an unregistered person and are never treated as sendable recipients. Suppression changes are append-only and auditable. The Institution 3 NERP promotion uses a fixed campaign key, rechecks suppression immediately before delivery, requires an explicit administrator confirmation, sends through SES, and records recipient-level sent/failed status and provider message IDs for idempotency.

**Unified AHA access pathways:** AHA BLS, ACLS, PALS, NRP, Instructor, and Heartsaver coursework is not free-entry. Access requires a valid NERP, IERP, approved ILSP AHA route, fully paid **Independent AHA Pathway**, or an audited, named-user administrator grant. The Independent AHA Pathway is the preferred name for non-cohort unsubsidized access and uses the published course prices. Existing learning progress is retained but cannot be read or continued until one of these entitlements is valid. NERP requires a current verified Nursing Council of Kenya licence with a licence number; IERP requires the submitted intern profile and MoH deployment/posting evidence before its programme record is created. A shareable access token is not used because it cannot provide safe identity, expiry, revocation, or audit controls.

**Global Admin entitlements:** migration `0147` adds a programme-aware, named-account entitlement ledger and immutable redemption history for IERP, NERP, ILSP, and self-pay learning. A Global Admin may select an existing Paeds Resus account or institution, record a reason, grant a full waiver or bounded percentage discount, set expiry and redemption limits, revoke unused access, and retain an internal audit reference. The reference is not a shareable learner token. IERP, NERP, and self-pay entitlements target one named user; ILSP entitlements target an institution-paid cohort order only and cannot create individual ILSP access. Entitlements adjust only the programme’s unpaid balance and never bypass clinical eligibility, evidence review, roster/readiness, practical assessment, certificate, or AHA credentialing gates. Existing payments are not rewritten or refunded automatically.

**Simplified Learning/CPD portal:** the institutional Learning workspace uses one top-level navigation: Learning overview, Cohorts & competency (IERS), CPD sessions, People & targets, and Reports & insights. CPD sessions has one canonical department-aware creation form, followed by a compact Sessions & Check-In operations view for QR attendance, certificates, and exports. People & targets owns department-scoped coordinator mapping and target follow-up; institution-wide staff-development appraisal remains institution-admin-only. Department coordinators never receive an unrestricted nurse list, and server-side reads filter staff, sessions, attendees, presenters, exports, and analytics to their assigned canonical departments. Legacy deep links remain compatible but do not create a second primary session-creation route.

**ERCo conflict confirmation and rejected roster records:** When a department mismatch is identified, the ERCo uses **Confirm conflict** to attest that CPD/profile evidence points to the selected department while the current institutional roster does not. The confirmation is reasoned, institution-scoped, audited, and routed to administrators; it does not assign an ERCo, UTL, ERTL, or ERT role. Administrators resolve the alert by reallocating the staff record to a confirmed canonical department or retiring the person. A rejected or roster-only staff record with no membership may be retired directly from People & profile through a reason-required, non-destructive staff-row action; this retains CPD and audit history without inventing a membership event. Membership-backed invited/active/suspended records continue through the normal membership removal workflow.

**Institutional member removal and scalable poles:** account administrators remove a person through an explicit, reason-required action in People & profile. Removal ends this institution’s membership and active product permissions, prevents future duty participation, and preserves historical attendance, accepted duties, readiness evidence, and audit records. The system blocks self-removal and protects the institution’s minimum administrator coverage; a removal event is append-only. ERT Step 2 supports any number of institution-defined poles, not only North and South. Poles have an explicit display order that administrators can arrange as North, South, East, West, or any local naming scheme; pole order is a management/navigation sequence and does not itself change clinical staffing or ERTL rotation unless the institution deliberately uses that pole’s department order. Each pole remains independently assignable to eligible departments.
...
The dated ERTL / Scene Commander is an automatic role: for each exact pole/date/shift, it is held by the explicitly staffed UTL of the leading confirmed operational department. The weekly rotation stores the leading department and its Monday-start sequence; it does not require an administrator to select an ERTL provider. The leading UTL must first accept the dated UTL duty, then receives a separate ERTL acceptance request. A provider may therefore hold separate `utl` and `ertl` rows for one shift while appearing once in the provider-facing team list; each scope retains its own explicit acceptance state. If the leading department has no valid explicit UTL, the team remains honestly incomplete rather than inventing staffing.

An accepted ERTL may assign a different supported responsibility to an existing ERT-member assignment or approve a provider recommendation. Direct assignment, approval, and role switching reset the affected assignment to `pending_acceptance`, clear prior acceptance/decline state, notify the provider, and append an auditable role event. The platform does not allow the ERTL to add arbitrary providers outside the published team through this control. Only an accepted ERTL on the current published/active team may coordinate these changes; active membership, tenant/pole/team scope, and assignment ownership remain mandatory.

The Individual provider dashboard presents a visible but non-submitting **Immediate ERT activation** action on Today. The confirmation form opens only after deliberate selection, captures the current team, activation type, location, optional bed number, and note, and then opens ResusGPS with the activation context. My Shift opens the exact currently active team first; future teams are available only through deliberate selection. Today uses the greeting **Welcome back** and does not duplicate the platform name or explanatory tagline already provided by the global header.

ResusGPS is a neonatal and paediatric reference-support product; adult pathways are not implied. The canonical bedside flow must treat patient weight as unknown until verified, must never substitute a default weight for dose-bearing content, and must fail closed for impossible values. Starting ABCDE/XABCDE assessment may remain immediate, but calculated medication and definitive-care content require a verified weight. New Case clears shared patient demographics and starts a new case-scoped analytics identity. ResusGPS, IERS targeted reporting, Care Signal post-event loops, and Fellowship evidence may share the case key needed for audit and adaptive learning, but they remain distinct products and must not store patient identifiers in analytics payloads.

Institution administrators may adjust the leadership department for the current or a future Monday-start week. The override recalculates the pole sequence and refreshes only future unaccepted rotation/team projections; completed weeks and accepted or historical duties are not rewritten. An institutional administrator, the department ERCo, or the accepted ERTL for the exact team may cancel an erroneous future UTL assignment with a required reason. Cancellation is non-destructive: it ends the future active assignment, clears future readiness state, records audit and role events, notifies the affected provider, and removes the row from active staffing while retaining history. Live or historical duties cannot be canceled through this control; a canceled future row can be deliberately reused only as a new pending-acceptance assignment.

The Individual provider dashboard presents the dated ERT first, then activation response, readiness, scheduled/in-progress simulation participation, evidence/actions, and named targeted reporting. Anonymous Care Signal and Code Signal remain separate independent reporting pathways. Provider drill discovery is limited to active institution memberships with IERS workspace access; joining remains separately gated by the IERS drills-operate capability and in-progress simulation status. These surfaces improve discoverability only and do not create real activations, drills, staffing, acceptance, readiness sign-off, or patient data.
