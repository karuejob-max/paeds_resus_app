...
**Consent-based CPD facility relationship:** every authenticated CPD registration records whether the host is the provider's `permanent_facility` or a `locum_outreach` site. A non-student Staff/RN provider may confirm a CPD host as permanent; this creates or accepts only a general `institutionMemberships` relationship and links the institution-scoped staff row, never an IERS product role, dated duty, team publication, or readiness sign-off. Existing suspended, ended, rejected, removed, or conflicting permanent relationships remain administrator-controlled. Outreach/locum attendance remains CPD history and does not move the provider's primary facility or activate membership. Providers with historical CPD attendance can make the same explicit confirmation from their CPD history page; the institution sees the resulting linked or pending state in People & roles.
