...
**Canonical department identity:** `facility_departments` is the institution-scoped source of truth for departments and their optional facility poles. IERS leads assign exactly one current ERCo per department and place departments into poles from this registry. CPD registration must offer the same institution’s configured department list and persist `facilityDepartmentId` on new CPD attendance and linked institutional staff rows. Historical department text remains for reporting compatibility; migration `0113` backfills only exact case-insensitive institution-scoped matches and never guesses unmatched legacy text. CPD and IERS remain independently gated products, but they intentionally share department identity where both are active.
