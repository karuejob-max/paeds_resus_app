...
- **Redacted-Derivative Design:** Soon after submission, an automated background job processes the free-text narrative field using an LLM on our paid, private tier. This job generates a `redacted_narrative` by stripping out direct identifiers (names, phone numbers, exact dates, facility names, IDs). Standard platform dashboards, facility-level rollups, and the automated pattern-detection pipeline (Section 3.4's Failure/Success Pattern Knowledge Base) read only this redacted derivative, never the original free-text narrative.
