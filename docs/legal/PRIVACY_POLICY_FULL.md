...
- **Redacted-Derivative Design:** Soon after submission, an automated background job processes the free-text narrative field using an LLM on our paid, private tier. This job generates a `redacted_narrative` by stripping out direct identifiers (names, phone numbers, exact dates, facility names, IDs). **As of this writing, no platform dashboard, facility-level rollup, or the automated pattern-detection pipeline (Section 3.4's Failure/Success Pattern Knowledge Base) actually reads the free-text narrative field at all — they operate on structured classifiers (event type, delays, outcome bands) only.** The redacted derivative exists in preparation for the point at which any of these features is extended to read narrative text; our design commitment is that when that happens, it will read only the redacted derivative, never the original. This paragraph will be updated to describe that as a present fact once a specific feature actually does so, rather than stating it as already true.
- **Separate, narrower internal tool:** an admin-only diagnostic feature ("AI Pattern Discovery") additionally analyses several other structured event fields (clinical presentation, system gaps, gap details, outcome) using a lighter, automated scrub (email addresses, phone numbers, and common clinician-title patterns) rather than the full LLM-based redaction pass described above, before sending that text to the same paid, private-tier LLM. This tool is restricted to platform administrators and is not part of any facility-facing dashboard or report. We are working to bring this feature's inputs up to the same redaction standard as the narrative field; until then, this paragraph reflects its current, narrower scrub.

**Retention and erasure — anonymise, not delete.** Care Signal reports are never hard-deleted, either at the end of the standard retention window (Section 11) or in response to a data-subject erasure request. Instead, the identity link (`userId`) is removed, and the `raw_narrative` is permanently overwritten and replaced by the redacted narrative derivative. This is a deliberate CEO decision to preserve the clinical/QI value of an already-filed report while still honouring an erasure request's practical effect — it remains subject to full counsel review before being treated as final policy (see Section 2 of the internal `LEGAL_SIGNOFF_BACKLOG.md` tracker).
...
| **Automated LLM Service Providers** (e.g. Google LLC or other API-compatible model providers) | Automated LLM features (narrative redaction, clinical tutor, feedback) | USA / EU / Global (Paid tier contractually restricts data use for model training or human review) |
...
| 1.2.0 | 2026-07-29 | Corrects an overclaim found in code review: §3.4's "Redacted-Derivative Design" bullet stated that platform dashboards, facility rollups, and the FPKB pattern-detection pipeline "read only this redacted derivative, never the original" — checked against the actual code, none of those three currently reads the free-text narrative field at all (they operate on structured classifiers only), so the claim was describing an intended future state as present-tense fact with no code-level guarantee behind it. Reworded to state the current reality plainly and frame the redacted-derivative design as a forward-looking commitment for when a feature is extended to read narrative text, not a guarantee already in force. Not re-consent-triggering — corrects the accuracy of an existing disclosure, doesn't change what's collected or why. See `LEGAL_SIGNOFF_BACKLOG.md` item 2.3. |
