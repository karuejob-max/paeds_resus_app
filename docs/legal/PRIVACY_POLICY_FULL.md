...
- **Redacted-Derivative Design:** Soon after submission, an automated background job processes the free-text narrative using an LLM on our paid, private tier. This job generates a `redacted_narrative` by stripping out direct identifiers (names, phone numbers, exact dates, facility names, IDs). All platform features, dashboards, and analyses read only this redacted derivative.
