...
| Item | Period |
|------|--------|
| Individual Care Signal events | **7 years** from submission, then **anonymised, not deleted** — the provider link (if any) is removed, and the `raw_narrative` is permanently overwritten and replaced by a redacted narrative derivative generated by an automated LLM redaction pass (contractually restricted under paid/private tiers to prevent model training or human review). The same treatment applies to an erasure request received before the 7-year mark. |
