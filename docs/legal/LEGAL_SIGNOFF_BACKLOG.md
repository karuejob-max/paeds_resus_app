...
### 2.3 Third-party name redaction in free text — LLM-based pass now built, still needs counsel sign-off on the as-built mechanism

**Update 2026-07-29:** this item was written as an open question ("is manual review sufficient, or does volume justify an LLM pass?") — that question has been overtaken by events. An LLM-based redaction pass has since been built and shipped (migration 0076, `server/lib/care-signal-redact.ts`, `server/scheduler.ts`'s 10-minute cron), using Google Gemini on the paid/private tier specifically so submitted narratives aren't used for model training. It's currently paused platform-wide behind a kill switch (`LLM_FEATURES_ENABLED`, see `server/_core/env.ts`) pending confirmation that Cloud Billing (not just a consumer subscription) is genuinely active on the underlying Google Cloud project. The original pattern-based redaction (`care-signal-deidentify.ts`) remains as the fallback for any narrative the LLM pass hasn't gotten to yet by the time it reaches anonymisation, and for narratives that fail LLM redaction repeatedly (`redactionAttempts` exceeding the retry limit — see migration 0081's retry-state tracking). **What's still open for counsel specifically:** (1) confirm the Gemini paid-tier data-processing terms are the right level of protection for clinical narrative content, versus needing Vertex AI's stronger enterprise terms (EU data residency, formal DPA) — see `PRIVACY_POLICY_FULL.md` §8's sub-processor table, written generically to allow switching providers without another legal-doc rewrite if that's the answer; (2) confirm the pattern-based fallback's narrower coverage (structured identifiers only, not names in free prose) is an acceptable interim state for the narratives it still covers.
