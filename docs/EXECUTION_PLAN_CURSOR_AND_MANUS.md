...
| # | Task ID | Title | Spec summary | Where | Status |
|---|---------|--------|----------------|-------|--------|
| 7 | **B2** | Institutional onboarding → portal | After onboarding success, redirect to `/institutional-portal`. Optionally add welcome/first-time state in InstitutionalPortal. | `client/src/pages/InstitutionalOnboarding.tsx`, `client/src/pages/InstitutionalPortal.tsx` | ⬜ Not started |
| 8 | **C3** | Conversion funnel in admin | Enrolled vs completed this month; show card e.g. "Enrolled: N → Completed: M (X%)". | `server/routers/admin-stats.ts`, `client/src/pages/AdminReports.tsx` | ⬜ Not started |
