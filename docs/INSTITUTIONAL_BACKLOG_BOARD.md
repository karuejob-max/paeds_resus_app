...
|----|--------|----------|--------|------------|--------|
| INST-18 | `individualInstallmentPayments` audit trail — **not "wire a writer," see 2026-07-31 finding** | P2 | Medium | 3 | Investigated 2026-07-31: this table was the intended dedicated instalment ledger per the original design doc, but the balance logic that shipped sums the generic `payments` table instead — this table has never had a writer, and building one now would just create a second, divergent ledger. Real decision needed (flagged in `schema.ts` comment, not resolved): either add `payments`' missing `phoneNumber`/typed-receipt columns and retire this table, or commit to writing here for real. Not a build task until that's decided. |
| INST-19 | Instructor pipeline scaling | P1 | Critical | 2 | **Partially shipped 2026-07-21**: per-course competency (`instructorQualifications`) and mentorship tiers (Provisional -> Qualified -> Lead Instructor, `instructorMentorships`/`instructorMentorshipGroups`) now built — see AGENTS.md §10. **Still open**: CEO's own method for actually recruiting/creating more instructors faster, not yet shared/built. The mechanics for using and progressing instructors well now exist; the supply-side bottleneck itself doesn't yet have a fix. Still blocks Kenya-wide spread and EAC expansion. |
...
| INST-25 | Implement the IERP/NERP v2 program respec (2026-07-31) | P1 | Critical | 1 | **Not started — this is a full spec, not yet built.** See `docs/IERP_NERP_PROGRAM_V2_SPEC.md` §8 for the complete gap list: Phase 1 upload gate (block elearning.heart.org cert upload until platform cognitive modules done, direct correctly both ways), Phase 2 role-based booking (team leader + 6 named team-member roles + up to 7 observers, replacing the flat capacity model — the current `simulationRole` enum only has 2 values), instructor-declared availability with first-declared-first-open session ordering, post-session instructor confirmation before a role counts + retrospective role-fill-in claims with instructor approval, admin override on all of it, a full payment-gating rewrite (nurses get a free BLS period now, interns' rules mostly unchanged), progress-bar + payment-ledger learner UI, removing `hybrid` as a cohort-program session type, and renaming "Nurse Cohort Program" to "Nurses Emergency Readiness Program (NERP)" throughout. Large enough it should probably be sequenced into its own set of PRs rather than one, once picked up. |
