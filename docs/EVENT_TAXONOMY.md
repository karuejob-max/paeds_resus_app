...
| `holistic_loop` | ResusGPS → Care Signal → learning closed-loop steps | `CareSignalPostEventPrompt.tsx`, `care-signal-events.ts` | diagnosis, mappedEventType, source, destination, careSignalEventId, actionLogId | Event names: `care_signal_prompt_shown`, `care_signal_prompt_dismissed`, `care_signal_prompt_accepted`, `care_signal_institutional_action_prompted`, `septic_shock_micro_course_clicked`. Mission KPI — separate from `provider_conversion`. |
...
**Form v2 (2026-05-17):** Structured QI payload in `careSignalEvents.gapDetails` (`formVersion: "v2"`) — report type, care location, timeline delays, equipment gaps, preventability, contributing factors, `proposedSystemFix`. Maps to legacy `eventType` / `outcome` / `systemGaps` / `chainOfSurvival` for fellowship and rollups. Facility dashboards parse v2 for preventable counts and top equipment gaps (`facility-care-signal.service`).
