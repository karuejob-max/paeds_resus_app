...
|------|-----|------|----------|
| 2026-04-01 | Cursor | **Safe-Truth age + home hub:** Parent and provider Safe-Truth forms support **neonate / infant (months) / child (years)**; `childAge` sent as approximate **months** with full detail in `presentation` (`client/src/lib/safetruth-age.ts`). **Provider `/safe-truth`** copy clarifies provider-only vs parent story. **`/home`:** first card opens **ResusGPS** (`/resus`) instead of `/` (was redirect loop); second card **Safe-Truth (providers)** → `/safe-truth`. **`safeTruthEvents.logEvent`:** allow `parent-observation` when `userType === parent`. | `630ebdd` |
