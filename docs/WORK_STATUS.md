...
|------|-----|------|----------|
| 2026-04-01 | Cursor | **Security baseline + B2B instructor assignment (before v4):** Admin audit log uses **parsed procedure `input`** for key summary (`summarizeAdminProcedureInput` in `server/_core/trpc.ts`); SECURITY_BASELINE.md updated. **B2B:** `users.instructorApprovedAt` + `pnpm run db:apply-0027` / `drizzle/0027_*.sql`; `adminStats.setInstructorApproval`; Admin → Reports user list Approve/Revoke; `institution.listAssignableInstructors`; `createTrainingSchedule` / `updateTrainingSchedule` accept `instructorUserId` (approved users only); `getTrainingSchedules` joins instructor name. Hospital Admin schedule form + session table updated. | (this commit) |
