...
|------|-----|------|----------|
| 2026-04-01 | Cursor | **Institutional onboarding UX + React #185:** Program step used parent `onClick` + Checkbox `onCheckedChange` (double toggle → max update depth). Replaced with `<label>` + single `onCheckedChange`. **Select** uses `undefined` when type unset (Radix). Theme: brand surfaces, `cta` buttons, calmer copy; country editable with Kenya default; hero no longer BLS/ACLS-only. | (this commit) |
