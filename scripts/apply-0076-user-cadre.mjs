/**
 * Idempotent: User cadre columns and cneAttendees cadre type upgrade (migration 0076).
 *
 * Adds:
...
async function main() {
  ...
  console.log("[0076] Connecting to database...");
  const conn = await createMysqlConnection(databaseUrl, mysql);

  // 1. Add cadre and cadreOther columns to users table
  if (!(await tableExists(conn, "users"))) {
    console.error("[0076] users table missing — run earlier migrations first.");
    process.exit(1);
  }

  if (await columnExists(conn, "users", "cadre")) {
    console.log("[0076] column users.cadre already exists — skip.");
  } else {
    await conn.query(
      `ALTER TABLE \`users\`
       ADD COLUMN \`cadre\` varchar(128) NULL DEFAULT NULL`
    );
    console.log("[0076] added column users.cadre.");
  }

  if (await columnExists(conn, "users", "cadreOther")) {
    console.log("[0076] column users.cadreOther already exists — skip.");
  } else {
    await conn.query(
      `ALTER TABLE \`users\`
       ADD COLUMN \`cadreOther\` varchar(128) NULL DEFAULT NULL`
    );
    console.log("[0076] added column users.cadreOther.");
  }

  // 2. Modify cneAttendees.cadre type to varchar(128)
  if (!(await tableExists(conn, "cneAttendees"))) {
    console.error("[0076] cneAttendees table missing — run earlier migrations first.");
    process.exit(1);
  }

  const cadreType = await getColumnType(conn, "cneAttendees", "cadre");
  if (cadreType === "varchar") {
    console.log("[0076] column cneAttendees.cadre is already varchar — skip.");
  } else {
    await conn.query(
      `ALTER TABLE \`cneAttendees\`
       MODIFY COLUMN \`cadre\` varchar(128) NOT NULL`
    );
    console.log("[0076] altered column cneAttendees.cadre type to VARCHAR(128).");
  }

  console.log("[0076] User cadre database migration completed.");
  await conn.end();
}
