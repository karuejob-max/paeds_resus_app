/**
 * Historical one-off: repair __drizzle_migrations after a mistaken 0034 apply, then apply 0035/0036.
 * Run from repo root: `node scripts/archive/fix-drizzle-migrations-oneoff.mjs`
 * Requires `.env` with DATABASE_URL. Do not use for routine deploys; use drizzle-kit migrate.
 */
...
await conn.query(
  "DELETE FROM `__drizzle_migrations` WHERE `hash` = 'fe379de657f2fbd25d39fdb9d9cb58e154515aa289d615964d104b955e6b2659'",
);
...
const sql0035 = readFileSync(join(ROOT, "drizzle/0035_worthless_famine.sql"), "utf8");
...
try {
  ...
  if (e.code === "ER_TABLE_EXISTS_ERROR") {
    ...
    await conn.query("INSERT IGNORE INTO `__drizzle_migrations` (`hash`, `created_at`) VALUES (?, ?)", [
      hash0035,
      1776872061837,
    ]);
  } else {
...
const sql0036 = readFileSync(join(ROOT, "drizzle/0036_safe_wilson_fisk.sql"), "utf8");
